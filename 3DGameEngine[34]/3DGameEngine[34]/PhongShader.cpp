...
PhongShader::PhongShader(const std::string& fileName)
{
...
//void PhongShader::Update(float value)
//{
//	glUniform1f(m_uniforms[FLOAT_U], value);
//}

GLuint PhongShader::CreateShader(const std::string& text, GLenum shaderType)
{
...
std::string PhongShader::LoadShader(const std::string& fileName)
{
	std::ifstream file;
	...
	{
		std::cerr << "Unable to load PhongShader: " << fileName << std::endl;
	}

	return output;
}

void PhongShader::CheckShaderError(GLuint shader, GLuint flag, bool isProgram, const std::string& errorMessage)
{
	GLint success = 0;
	GLchar error[1024] = { 0 };
